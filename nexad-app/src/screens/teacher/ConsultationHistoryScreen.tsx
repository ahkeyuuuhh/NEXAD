...
export default function ConsultationHistoryScreen({ navigation }: any) {
  ...
  const getStatusDisplay = (consultation: ConsultationWithStudent) => {
    if (consultation.status === 'completed') {
      return { text: 'Done', color: '#166534', bgColor: '#DCFCE7' };
    }
                  ...
                  <View style={styles.selectRow}>
                    ...
                    <View style={{ flex: 1 }}>
                      <View style={styles.cardTopRow}>
                        <Text style={styles.studentText}>{consultation.studentName}</Text>
...
const styles = StyleSheet.create({
  ...
  cardSelected:         { backgroundColor: C.surfaceAlt, borderColor: C.ink3 },
  selectCheckbox: {
  ...
  selectCheckboxActive: { backgroundColor: C.action, borderColor: C.action },
  swipeDeleteAction: {
  ...
  selectDeleteBtnText:  { color: '#fff', fontSize: 15, fontWeight: '600' as const },
